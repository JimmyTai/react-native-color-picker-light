...
public class RCTWhitePickerManager extends SimpleViewManager {
  ...
  public RCTWhitePickerManager(ReactApplicationContext reactContext) {
  ...
  public void receiveCommand(@NonNull View root, int commandId, @Nullable ReadableArray args) {
    ...
    try {
      if (root instanceof WhiteColorPicker && args != null) {
        switch (commandId) {
          case COMMAND_SHOW_COLOR:
            if (args.isNull(0)) break;
            String hex = args.getString(0);
            ((WhiteColorPicker) root).setColor(Color.parseColor(hex));
            break;
        }
      }
    } catch (Exception e) {
